...
class Component extends DCLogic {
  ...
  recipes = {
    ...
    'Turkey Bean Vegetable Pasta': { cuisine:'Weeknight · Lunch', time:'25 min', kcal:590, base:2, weeklyReference:true, weeklyReferenceDays:1, fixedPlan:true, protein:70, fiber:17, slug:'turkey-bean-pasta', ingredients:[{n:'Turkey mince',q:300,u:'g',cat:'protein',prot:66,short:'Turkey'},{n:'Wholewheat pasta',q:180,u:'g',cat:'carb',prot:23},{n:'Cannellini beans',q:240,u:'g',cat:'protein',prot:16},{n:'Courgette',q:1,u:'',cat:'veg',prot:2},{n:'Cherry tomatoes',q:250,u:'g',cat:'veg',prot:3},{n:'Tomato passata',q:300,u:'g',cat:'sauce',prot:4}], method:['Cook pasta until just tender.','Brown turkey mince, then cook courgette and tomatoes until softened.','Add passata and beans, simmer 8 minutes, then toss with pasta.'] },
  ...
  groceryFor() {
    ...
    if (!plan) {
      ...
      const q=(name, unit='g')=>Math.round(amount(name))+(unit?' '+unit:'');
      return { label:'Wednesday–Sunday Meal Prep', skipped:[], groups:[
      {h:'PROTEINS & DAIRY', items:[{n:'Turkey mince',q:q('Turkey mince'),img:'gr-chicken'},{n:'Greek yogurt',q:q('Greek yogurt')},{n:'Cottage cheese',q:q('Cottage cheese')},{n:'Milk',q:q('Milk')},{n:'Protein powder',q:q('Protein powder','scoops')}]},
      {h:'PRODUCE & CARBS', items:[{n:'Bananas',q:q('Bananas','')},{n:'Mixed berries',q:q('Mixed berries')},{n:'Courgettes',q:q('Courgette','')},{n:'Cherry tomatoes',q:q('Cherry tomatoes')},{n:'Red peppers',q:q('Red pepper','')},{n:'Potatoes',q:q('Potatoes')},{n:'Rolled oats',q:q('Rolled oats')},{n:'Wholewheat pasta',q:q('Wholewheat pasta')},{n:'Chia seeds',q:q('Chia seeds')},{n:'Pumpkin seeds',q:q('Pumpkin seeds')}]},
      {h:'CUPBOARD · ONLY IF NEEDED', items:[{n:'Cannellini beans',q:q('Cannellini beans')},{n:'Kidney beans',q:q('Kidney beans')},{n:'Tomato passata',q:q('Tomato passata')},{n:'Chopped tomatoes',q:q('Chopped tomatoes')},{n:'Peanut butter',q:q('Peanut butter')}]},
    ]};
    }
  ...
  ingredientsPanel() {
    ...
    // Whole produce cannot be shopped as an opaque decimal. Round it up visibly;
    // the underlying profile-responsive calculation remains exact for every other ingredient.
    const fmtIngredient = (v, unit)=> unit ? fmt(v)+' '+unit : Math.max(1,Math.ceil(v))+' whole';
    ...
    const personHead = (p, scale, sp) => e('div',{key:p.name,style:{flex:1,background:p===me?'#F6ECE1':'#F3EBDD',borderRadius:14,padding:'12px 12px'}},
      ...
      e('div',{style:{display:'flex',flexDirection:'column',gap:4}},
        mainItems.map(ing=>e('div',{key:ing.n,style:{display:'flex',justifyContent:'space-between',fontFamily:"'Hanken Grotesk',sans-serif",fontSize:12,color:'#57493C'}},
          ...
          e('span',{style:{fontFamily:"'JetBrains Mono',monospace",fontSize:11}},Math.round(ing.q/((r.base||2)*(r.weeklyReference?(r.weeklyReferenceDays||3):1))*scale)+'g'))),
