...
const expected = Math.ceil(changedLunch.totalIngredients['Turkey mince'] + changedDinner.totalIngredients['Turkey mince']) + ' g';
assert.strictEqual(app.groceryFor().groups.flatMap((group)=>group.items).find((item)=>item.n==='Turkey mince').q, expected, 'cart must use the changed combined turkey weight');
